$(function () {
                    ...
                    render: function(data, type, row, meta)
                    {
                        var actions = `
                            <div class="form-group col-md-12">
                                <input type="number" class="form-control required poll" placeholder="Poll" value="${row["primary_poll"] || 0}" data-election-id="${row["election_id"]}" data-candidate-id="${data}" data-polling-unit-id="${pollingUnitID}">
                            </div>
                        `;
    ...
    function loadPollingUnits()
    {
        ...
        $("select.state").on("change", function(){
            ...
            {
                $('select.polling_unit_id').html(`<option value="">Please Select</option>`);
                $('selectpicker').selectpicker('refresh');
            }
        })
